...
export function formatShortDate(time) {
    ...
    const monthString = (day.getMonth() + 1) < 10 ? `0${day.getMonth() + 1}` : (day.getMonth() + 1);
...
export const devideNoProgramByHalfHour = (noProgram) => {
    ...
    if (deltalTime > TV_GUIDE_CONSTANTS.HALF_HOUR_DURATION) {
        const loop = Math.floor((deltalTime - deltalTime % TV_GUIDE_CONSTANTS.HALF_HOUR_DURATION) / TV_GUIDE_CONSTANTS.HALF_HOUR_DURATION);
        for (let j = 0; j < loop; j++) {
            const startDateEdited = noProgram.startDateAdjusted + j * TV_GUIDE_CONSTANTS.HALF_HOUR_DURATION;
            noProgramList.push({
                ...noProgram,
                startDate: startDateEdited,
                startDateAdjusted: startDateEdited,
                endDate: startDateEdited + TV_GUIDE_CONSTANTS.HALF_HOUR_DURATION,
                endDateAdjusted: startDateEdited + TV_GUIDE_CONSTANTS.HALF_HOUR_DURATION
            });
        }
        if (deltalTime % TV_GUIDE_CONSTANTS.HALF_HOUR_DURATION !== 0) {
            noProgramList.push({
                ...
                startDateAdjusted: noProgramList[noProgramList.length - 1].endDateAdjusted,
        ...
        return programListData.map((channel, indexChannel) => {
            ...
            programs.map((programItem, indexProgram) => {
                ...
                if (programItem && programItem.id === -1) {
                    ...
                    const newNoProgramItemProcessed = devideNoProgramByHalfHour(programItem);
                    newNoProgramList = [...newNoProgramList, ...newNoProgramItemProcessed];
                }
                if ((indexProgram !== programs.length - 1) && programItem.endDateAdjusted < programs[indexProgram + 1].startDateAdjusted) {
            ...
            dataPrograms.sort((program1, program2) => program1.startDateAdjusted - program2.startDateAdjusted);
...
export const getIndexProgramNextFocus = ({ programs, timeLineShow }) => {
    let data = [...programs];
    const timeAVG = (timeLineShow[0].start + timeLineShow[timeLineShow.length - 1].start) / 2;
    data.sort((program1, program2) => {
        return (program1.startDateAdjusted - timeAVG) - (program2.startDateAdjusted - timeAVG);
    });
    const minStartDateAdjustedProgram = data[0];
    return programs.findIndex(program => program.startDateAdjusted === minStartDateAdjustedProgram);
};

export const getIndexOfProgramNextFocus = ({ programs, programActive, lineIndex }) => {
    try {
        ...
        programListClone.map((program, index) => {
            ...
            if (startDateAdjusted <= programActive.startDateAdjusted && endDateAdjusted >= programActive.endDateAdjusted) {
                indexNextActive = index;
            }
        });
        if (indexNextActive === 0) {
            const nextProgramClosestStartDateAdjusted = programListClone.reduce((program1, program2) => {
                return Math.abs(program2.startDateAdjusted - programActive.startDateAdjusted) < Math.abs(program1.startDateAdjusted - programActive.startDateAdjusted) ? program2 : program1;
            });
            indexNextActive = programListClone.findIndex(program => program.startDateAdjusted === nextProgramClosestStartDateAdjusted.startDateAdjusted);
        }
